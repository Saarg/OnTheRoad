namespace Controls {
	...
	public class MultiOSControls : MonoBehaviour {
		...
		static public MultiOSControls Instance { internal set; get; }
		...
		public void Start() {
			if (Instance == null) {
				...
				Debug.LogWarning("multiple os controls found!!!! deleting some.");
				Destroy (this);
			}
		}
		...
		public float getValue(string name) {
			for (int i = 0 ; i < _inputs.Length ; i++) {
				if (_inputs[i].name == name) {
					...
					#endif
					return Mathf.Clamp(_inputs[i].value, -1.0f, 1.0f);
				}
			}
			return 0;
		}
